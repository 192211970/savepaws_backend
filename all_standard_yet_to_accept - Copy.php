...
/*
|---------------------------------------------------------
| STANDARD CASES – REPORTED & PENDING
|---------------------------------------------------------
| - cases.status = Reported
| - case_type = Standard
| - no Accept exists
| - ignore Resent / Sent_again
|---------------------------------------------------------
*/
...
WHERE
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM case_escalations r
        WHERE r.case_id = c.case_id
          AND r.remark IN ('Resent', 'Sent_again')
    )
